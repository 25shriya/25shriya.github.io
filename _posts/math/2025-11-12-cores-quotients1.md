...
Now, for a $\beta$-set $X$ for a partition $\lambda$, fix a positive integer $p$ and define $X_i^{(p)} = \{a \in \mathbb{N}_0 \, \mid \, ap + i \in X\}$ and $X_{(p)} = \bigcup_{i=0}^{p-1} \{ap+i \, \mid \, 0 \leq a \leq \mid X_i^{(p)} \mid \}$. The $\textit{p-core}$ of $\lambda$ is $\lambda_{(p)} = P(X_{(p)})$ and $\textit{p-quotient}$ is $\lambda^{(p)} = (\lambda_0, \ldots, \lambda_{p-1})$, where $\lambda_i = P(X_i^{(p)})$.
...
4. Moreover, $X'_{(p)} = \bigcup_{i=0}^{p-1} \{ap+i \, \mid \, 0 \leq a \leq \mid X_i^{(p)'} \mid \} \implies \bigcup_{i=0}^{p-1} \{ap+i \, \mid \, 0 \leq a \leq \mid X_i^{(p)} \mid \} \implies \bigcup_{i=0}^{p-1} \{ap+i \, \mid \, 0 \leq a \leq \mid Y_i^{(p)} \mid \} = \kappa$. Hence, the $p$-core of the partition $P(X')$ is $\kappa$.
